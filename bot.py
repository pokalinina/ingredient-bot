...
PROMPT_TEMPLATE = """Ты дружелюбный эксперт по косметической безопасности. Знаешь ЕС №1223/2009 и ТР ТС 009/2011. Общаешься просто и понятно, как умный друг который разбирается в косметике.
...
Найди полный INCI-состав. Оцени ингредиенты по критериям: риск аллергии, эндокринный риск, канцерогенность, риск при высокой концентрации.

ПРАВИЛА ОЦЕНКИ:
— Ингредиенты в начале состава имеют большую концентрацию и больший вес
— Запрещённые ингредиенты снижают оценку согласно правилам выше
...
СТРОГИЕ ПРАВИЛА ФОРМАТИРОВАНИЯ — нарушать нельзя:
— НИКАКИХ звёздочек * в тексте вообще
— НИКАКИХ символов # в тексте вообще
— Уровни риска писать только так: Высокий риск / Средний риск / Низкий риск
— Только эмодзи из шаблона ниже, больше никаких
— Каждый раздел отделять пустой строкой
...
— [Ингредиент]: [простое объяснение] — [Высокий риск / Средний риск / Низкий риск]
...
Почему: [1-2 предложения с конкретными причинами, без звёздочек]

👤 Подходит ли тебе, {name}:
Совместимость: [да / частично / нет]
[2-3 предложения про совместимость с {skin_type} кожей и {skin_concerns}, без звёздочек]

💡 Твоя персональная рекомендация:
[Если оценка ниже 75 — что конкретно искать в составе альтернативного средства, какие ингредиенты предпочесть. Если оценка 75 и выше — практический совет по использованию с учётом типа кожи. Без звёздочек и решёток]"""
...
async def analyze(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Удаляем звёздочки и решётки на всякий случай
